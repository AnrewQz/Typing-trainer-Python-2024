...
def check(example, typed, text, stat, window):
    ...
    if example < len(text) and text[example] == typed:
        if example == len(text) - 1:
            ...
            stat.wpm = (stat.amount_of_words * seconds_in_minute) / (stat.current_time - stat.starting_time)
        ...
        stat.cpm = (stat.amount_of_symbols * seconds_in_minute) / (stat.current_time - stat.starting_time)
